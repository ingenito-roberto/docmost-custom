  ...
  addAttributes() {
    return {
      ...
      numbered: {
        default: true,
        parseHTML: (element) => {
          const val = element.getAttribute("data-numbered");
          // If the attribute is absent, default to true (backward compat)
          if (val === null) return true;
          return val !== "false";
        },
        renderHTML: (attributes) => ({
          "data-numbered": String(attributes.numbered ?? true),
        }),
      },
    };
  },
      ...
      toggleHeadingNumbered:
        ...
        ({ commands, editor }) => {
          if (editor.isActive("heading")) {
            const current = editor.getAttributes("heading").numbered !== false;
  ...
  renderHTML({ node, HTMLAttributes }) {
    ...
    return [
      ...
      mergeAttributes(this.options.HTMLAttributes, HTMLAttributes, {
        id: node.attrs.id,
        "data-numbered": String(node.attrs.numbered ?? true),
      }),
      0,
    ];
  },
